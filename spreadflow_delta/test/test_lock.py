...
class LockTestCase(TestCase):
    ...
    def test_lock_update_when_pending(self):
        """
        Test the lockfile processor when updates arrive on pending objects.
        """
        ...
        # Update when lock is unavaillable
        reinsert_proto = {
            'inserts': ['b'],
            'deletes': ['a'],
            'data': {
        ...
        reinsert = copy.deepcopy(reinsert_proto)
        update = copy.deepcopy(expected)
        matches = MatchesSendDeltaItemInvocation(expected, sut.out_retry)
        send = Mock(spec=Scheduler.send)
        ...
        # Retry-success lock
        lock_mock = Mock(spec=_Lockfile)
        expected = copy.deepcopy(reinsert_proto)
        expected['date'] = 'DATESTAMP'
        matches = MatchesSendDeltaItemInvocation(expected, sut.out_locked)
        send = Mock(spec=Scheduler.send)
